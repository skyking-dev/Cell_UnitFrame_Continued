...
local function GetAnchorFrameTargetText(anchorTarget)
    if anchorTarget == const.UNIT_FRAME_ANCHOR.SCREEN_CENTER then
        return L.ScreenCenter
    elseif anchorTarget == const.UNIT_FRAME_ANCHOR.PLAYER_FRAME then
        return L.PlayerFrame
    elseif anchorTarget == const.UNIT_FRAME_ANCHOR.TARGET_FRAME then
        return L.TargetFrame
    elseif anchorTarget == const.UNIT_FRAME_ANCHOR.FOCUS_FRAME then
        return L.FocusFrame
    elseif anchorTarget == const.UNIT_FRAME_ANCHOR.ESSENTIAL_CDS then
        return L.EssentialCDs
    elseif anchorTarget == const.UNIT_FRAME_ANCHOR.CUSTOM_FRAME then
        return L.ExistingFrame
    end

    return L[anchorTarget] or anchorTarget
end

---@param layout UnitLayout
---@return ParentAnchor
local function EnsureUnitAnchorPosition(layout)
    layout.anchorPosition = layout.anchorPosition or {
        point = "CENTER",
...
local function CreatePositioningPopup()
    ...
    local anchorFrameDropdown, customFrameBox

    ---@type CellCheckButton
    local anchorToParentCB = Cell.CreateCheckButton(parentAnchorFrame, "", function(checked)
        local layout = CUF.DB.CurrentLayoutTable()[positioningPopup.unit]
        layout.anchorToParent = checked
        EnsureUnitAnchorPosition(layout)
        CUF:Fire("UpdateLayout", nil, "position", positioningPopup.unit)

        anchorFrameDropdown:SetEnabled(checked)
        ...
        customFrameBox:SetEnabled(checked and anchorFrameDropdown:GetSelected() == const.UNIT_FRAME_ANCHOR.CUSTOM_FRAME)

        positioningPopup.xPosSlider:SetEnabled(not checked)
        positioningPopup.yPosSlider:SetEnabled(not checked)
        if positioningPopup.mirrorCB:IsShown() then
            positioningPopup.mirrorCB:SetEnabled(not checked)
        end
    end)
    ...
    ---@type CellDropdown
    anchorFrameDropdown = Cell.CreateDropdown(parentAnchorFrame, 264)
    anchorFrameDropdown:SetPoint("TOPLEFT", anchorToParentCB, "BOTTOMLEFT", 0, -30)
    anchorFrameDropdown:SetLabel(L.AnchorFrameTo)

    for _, anchorTarget in ipairs(const.UNIT_FRAME_ANCHOR_TARGETS) do
        anchorFrameDropdown:AddItem({
            text = GetAnchorFrameTargetText(anchorTarget),
            value = anchorTarget,
            onClick = function()
                local layout = CUF.DB.CurrentLayoutTable()[positioningPopup.unit]
                layout.parent = anchorTarget
                if layout.parentFrame == nil then
                    layout.parentFrame = ""
                end
                EnsureUnitAnchorPosition(layout)

                customFrameBox:SetShown(anchorTarget == const.UNIT_FRAME_ANCHOR.CUSTOM_FRAME)
                customFrameBox:SetEnabled(layout.anchorToParent and anchorTarget == const.UNIT_FRAME_ANCHOR.CUSTOM_FRAME)
                CUF:Fire("UpdateLayout", nil, "position", positioningPopup.unit)
            end,
        })
    end

    customFrameBox = CUF:CreateEditBox(parentAnchorFrame, 264, 20, L.CustomFrameName)
    customFrameBox:SetPoint("TOPLEFT", anchorFrameDropdown, "BOTTOMLEFT", 0, -30)
    local function SaveCustomFrameName(self)
        CUF.DB.CurrentLayoutTable()[positioningPopup.unit].parentFrame = self:GetText()
        CUF:Fire("UpdateLayout", nil, "position", positioningPopup.unit)
    end
    customFrameBox:SetScript("OnEnterPressed", function(self)
        self:ClearFocus()
        SaveCustomFrameName(self)
    end)
    customFrameBox:HookScript("OnEditFocusLost", SaveCustomFrameName)

    ---@type CellDropdown
    local pointDropdown = Cell.CreateDropdown(parentAnchorFrame, 117)
    pointDropdown:SetPoint("TOPLEFT", customFrameBox, "BOTTOMLEFT", 0, -30)
    pointDropdown:SetLabel(L.AnchorFrom)
    ...
    relativePointDropdown:SetLabel(L.AnchorTo)
    ...
    for _, point in pairs(const.ANCHOR_POINTS) do
        pointDropdown:AddItem({
            ...
            value = point,
            onClick = function()
                EnsureUnitAnchorPosition(CUF.DB.CurrentLayoutTable()[positioningPopup.unit]).point = point
                CUF:Fire("UpdateLayout", nil, "position", positioningPopup.unit)
            end,
        })
        relativePointDropdown:AddItem({
            ...
            value = point,
            onClick = function()
                EnsureUnitAnchorPosition(CUF.DB.CurrentLayoutTable()[positioningPopup.unit]).relativePoint = point
                CUF:Fire("UpdateLayout", nil, "position", positioningPopup.unit)
            end,
        })
    end
    ...
    parentOffsetXSlider.onValueChangedFn = function(value)
        EnsureUnitAnchorPosition(CUF.DB.CurrentLayoutTable()[positioningPopup.unit]).offsetX = value
        CUF:Fire("UpdateLayout", nil, "position", positioningPopup.unit)
    end
    ...
    parentOffsetYSlider.onValueChangedFn = function(value)
        EnsureUnitAnchorPosition(CUF.DB.CurrentLayoutTable()[positioningPopup.unit]).offsetY = value
        CUF:Fire("UpdateLayout", nil, "position", positioningPopup.unit)
    end

    positioningPopup.parentAnchorFrame = parentAnchorFrame
    positioningPopup.anchorFrameDropdown = anchorFrameDropdown
    positioningPopup.customFrameBox = customFrameBox
...
local function UpdatePositioningPopup()
    ...
    if layout.anchorToParent ~= nil then
        ...
        local anchorPosition = EnsureUnitAnchorPosition(layout)

        positioningPopup.anchorToParentCB:SetChecked(checked)
        positioningPopup.anchorToParentCB.label:SetText(L.AnchorFrameTo)

        local parentTarget = layout.parent or const.UNIT_FRAME_ANCHOR.SCREEN_CENTER
        positioningPopup.anchorFrameDropdown:SetSelectedValue(parentTarget)
        positioningPopup.customFrameBox:SetText(layout.parentFrame or "")
        positioningPopup.customFrameBox:SetShown(parentTarget == const.UNIT_FRAME_ANCHOR.CUSTOM_FRAME)

        positioningPopup.pointDropdown:SetSelectedValue(anchorPosition.point)
        positioningPopup.relativeDropdown:SetSelectedValue(anchorPosition.relativePoint)
        positioningPopup.parentOffsetXSlider:SetValue(anchorPosition.offsetX)
        positioningPopup.parentOffsetYSlider:SetValue(anchorPosition.offsetY)

        positioningPopup.anchorFrameDropdown:SetEnabled(checked)
        positioningPopup.pointDropdown:SetEnabled(checked)
        positioningPopup.relativeDropdown:SetEnabled(checked)
        positioningPopup.parentOffsetXSlider:SetEnabled(checked)
        positioningPopup.parentOffsetYSlider:SetEnabled(checked)
        positioningPopup.customFrameBox:SetEnabled(checked and parentTarget == const.UNIT_FRAME_ANCHOR.CUSTOM_FRAME)
    ...
    if positioningPopup.mirrorCB:IsShown() then
        positioningPopup.mirrorCB:SetEnabled(not layout.anchorToParent)
    end
end
...
local function ShowPositioningPopup(unit)
    ...
    if CUF.DB.CurrentLayoutTable()[unit].anchorToParent ~= nil then
        ...
        positioningPopup:SetHeight(330)
    else
