...
local function FormatPercent(max, cur)
    if cur == nil then return "0.00%" end
    if Util.HasAnySecretValues(max, cur) or max == 0 then return end
    if cur == 0 then return "0.00%" end
...
local function FormatNumberNoZeroes(num)
    if num == nil then return end
    if Util.IsValueNonSecret(num) and num == 0 then return end
...
local function FormatNumberShort(num)
    if num == nil then return end
    if not Util.IsValueNonSecret(num) then
...
local function CombineFormats(format1, format2, separator)
    if not format1 then return format2 end
...
---@param short boolean
---@return string?
local function GetHealthPercentText(unit, short)
...
---@return string?
local function GetPowerPercentText(unit, short, powerType)
    if not UnitPowerPercent then return end

    local pct
    if CurveConstants and CurveConstants.ScaleTo100 then
        pct = UnitPowerPercent(unit, powerType, true, CurveConstants.ScaleTo100)
    else
...
---@param value number?
---@return boolean
local function IsKnownZero(value)
    return Util.IsValueNonSecret(value) and value == 0
end

---@param ... number?
---@return boolean
local function CanDoArithmetic(...)
    for i = 1, select("#", ...) do
        local value = select(i, ...)
        if value == nil or not Util.IsValueNonSecret(value) then
            return false
        end
    end
...
local function SafeAdd(a, b)
    if not CanDoArithmetic(a, b) then return end
    return a + b
end
...
function W:WrapTagFunction(tag, prefix, suffix)
    ...
    local function CallTag(unit)
        if not Cell.isMidnight then
            return tag(unit)
        end

        local ok, result, aux = pcall(tag, unit)
        if ok then
            return result, aux
        end
    end
        ...
        if suffix then
            if positiveColor or negativeColor then
                return function(unit)
                    local result, isPositive = CallTag(unit)
                    if result ~= nil then
...
W:AddTag("perhp", "UNIT_HEALTH UNIT_MAXHEALTH", function(unit)
    if Util.HasAnySecretValues(UnitHealthMax(unit), UnitHealth(unit)) then
        return GetHealthPercentText(unit, false)
    end
...
W:AddTag("perhp:short", "UNIT_HEALTH UNIT_MAXHEALTH", function(unit)
    if Util.HasAnySecretValues(UnitHealthMax(unit), UnitHealth(unit)) then
        return GetHealthPercentText(unit, true)
    end
...
W:AddTag("defhp", "UNIT_HEALTH UNIT_MAXHEALTH", function(unit)
    local deficit = SafeSubtract(UnitHealth(unit), UnitHealthMax(unit))
    if deficit == nil or deficit == 0 then return end
...
W:AddTag("perdefhp", "UNIT_HEALTH UNIT_MAXHEALTH", function(unit)
    ...
    local deficit = SafeSubtract(current, maxhp)
    if deficit == nil or deficit == 0 then return end
...
W:AddTag("abs", "UNIT_ABSORB_AMOUNT_CHANGED", function(unit)
    ...
    if IsKnownZero(totalAbsorbs) then return end
...
W:AddTag("perhp:perabs", "UNIT_HEALTH UNIT_MAXHEALTH UNIT_ABSORB_AMOUNT_CHANGED", function(unit)
    ...
    local maxhp = UnitHealthMax(unit)
    local totalAbsorbs = UnitGetTotalAbsorbs(unit)
    local cur = FormatPercent(maxhp, curhp)
    if cur == nil then
        cur = GetHealthPercentText(unit, false)
    end

    return CombineFormats(cur, FormatPercentNoZeroes(maxhp, totalAbsorbs))
end, "Health", "75.20% + 15%")
W:AddTag("perhp:perabs:short", "UNIT_HEALTH UNIT_MAXHEALTH UNIT_ABSORB_AMOUNT_CHANGED", function(unit)
    local curhp = UnitHealth(unit)
    local maxhp = UnitHealthMax(unit)
    local totalAbsorbs = UnitGetTotalAbsorbs(unit)
    local cur = FormatPercentShort(maxhp, curhp)
    if cur == nil then
        cur = GetHealthPercentText(unit, true)
    end

    return CombineFormats(cur, FormatPercentShortNoZeroes(maxhp, totalAbsorbs))
end, "Health", "75% + 15%")
...
W:AddTag("curhp:abs:merge", "UNIT_HEALTH UNIT_ABSORB_AMOUNT_CHANGED", function(unit)
    local curhp = UnitHealth(unit)
    local totalAbsorbs = UnitGetTotalAbsorbs(unit)
    local total = SafeAdd(curhp, totalAbsorbs)
    if total == nil then
        if IsKnownZero(totalAbsorbs) then
            return FormatText(curhp)
        end
        return
    end

    return FormatText(total)
end, "Health", "40000")
W:AddTag("curhp:abs:merge:short", "UNIT_HEALTH UNIT_ABSORB_AMOUNT_CHANGED", function(unit)
    ...
    if total == nil then
        if IsKnownZero(totalAbsorbs) then
            return FormatText(curhp, nil, false, true)
        end
        return
    end

    return FormatText(total, nil, false, true)
end, "Health", "40K")
W:AddTag("perhp:perabs:merge", "UNIT_HEALTH UNIT_MAXHEALTH UNIT_ABSORB_AMOUNT_CHANGED", function(unit)
    ...
    if total == nil then
        if IsKnownZero(totalAbsorbs) then
            return FormatText(maxhp, curhp, true) or GetHealthPercentText(unit, false)
        end
        return
    end

    return FormatText(maxhp, total, true)
end, "Health", "90.20%")
W:AddTag("perhp:perabs:merge:short", "UNIT_HEALTH UNIT_MAXHEALTH UNIT_ABSORB_AMOUNT_CHANGED", function(unit)
    ...
    if total == nil then
        if IsKnownZero(totalAbsorbs) then
            return FormatText(maxhp, curhp, true, true) or GetHealthPercentText(unit, true)
        end
        return
    end

    return FormatText(maxhp, total, true, true)
end, "Health", "90%")
W:AddTag("abs:healabs:merge", "UNIT_HEAL_ABSORB_AMOUNT_CHANGED UNIT_ABSORB_AMOUNT_CHANGED", function(unit)
    ...
    local totalAbsorbs = UnitGetTotalAbsorbs(unit)
    local tot = SafeSubtract(totalAbsorbs, totalHealAbsorbs)
    if tot == nil or tot == 0 then return end
...
W:AddTag("healabs", "UNIT_HEAL_ABSORB_AMOUNT_CHANGED", function(unit)
    local totalHealAbsorbs = UnitGetTotalHealAbsorbs(unit)
    if IsKnownZero(totalHealAbsorbs) then return end
...
W:AddTag("perpp", "UNIT_POWER_FREQUENT UNIT_DISPLAYPOWER UNIT_MAXPOWER", function(unit)
    if Util.HasAnySecretValues(UnitPowerMax(unit), UnitPower(unit)) then
        return GetPowerPercentText(unit, false)
    end
...
W:AddTag("perpp:short", "UNIT_POWER_FREQUENT UNIT_DISPLAYPOWER UNIT_MAXPOWER", function(unit)
    if Util.HasAnySecretValues(UnitPowerMax(unit), UnitPower(unit)) then
        return GetPowerPercentText(unit, true)
    end
...
W:AddTag("defpp", "UNIT_POWER_FREQUENT UNIT_DISPLAYPOWER UNIT_MAXPOWER", function(unit)
    local deficit = SafeSubtract(UnitPower(unit), UnitPowerMax(unit))
    if deficit == nil or deficit == 0 then return end
    return FormatText(deficit)
end, "Power", "-3000")
...
W:AddTag("perdefpp", "UNIT_POWER_FREQUENT UNIT_DISPLAYPOWER UNIT_MAXPOWER", function(unit)
    ...
    local deficit = SafeSubtract(UnitPower(unit), max)
    if deficit == nil or deficit == 0 then return end
...
W:AddTag("curmana", "UNIT_POWER_FREQUENT UNIT_DISPLAYPOWER", function(unit)
    if IsKnownZero(UnitPowerMax(unit, MANA_POWER_TYPE)) then return end
...
W:AddTag("permana", "UNIT_POWER_FREQUENT UNIT_DISPLAYPOWER UNIT_MAXPOWER", function(unit)
    ...
    if IsKnownZero(max) then return end
    if Util.HasAnySecretValues(max, UnitPower(unit, MANA_POWER_TYPE)) then
        return GetPowerPercentText(unit, false, MANA_POWER_TYPE)
    end
...
W:AddTag("permana:short", "UNIT_POWER_FREQUENT UNIT_DISPLAYPOWER UNIT_MAXPOWER", function(unit)
    ...
    if IsKnownZero(max) then return end
    if Util.HasAnySecretValues(max, UnitPower(unit, MANA_POWER_TYPE)) then
        return GetPowerPercentText(unit, true, MANA_POWER_TYPE)
    end
...
W:AddTag("defmana", "UNIT_POWER_FREQUENT UNIT_DISPLAYPOWER UNIT_MAXPOWER", function(unit)
    local max = UnitPowerMax(unit, MANA_POWER_TYPE)
    if IsKnownZero(max) then return end
    local deficit = SafeSubtract(UnitPower(unit, MANA_POWER_TYPE), max)
    if deficit == nil or deficit == 0 then return end
    return FormatText(deficit)
end, "Power", "-3000")
...
W:AddTag("curaltmana", "UNIT_POWER_FREQUENT UNIT_DISPLAYPOWER", function(unit)
    if UnitPowerType(unit) == MANA_POWER_TYPE
        or IsKnownZero(UnitPowerMax(unit, MANA_POWER_TYPE)) then
        return
    end
    return FormatText(UnitPower(unit, MANA_POWER_TYPE))
end, "Power", "12000")
