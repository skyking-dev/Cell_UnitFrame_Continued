...
local function PowerFilterCheck(self)
    ...
    local guid = owner.states.guid
    if guid == nil then
        guid = Util.GetReadableGUID(owner.states.unit)
    end
    if guid == nil then
        return true
    end
...
local function Enable(self)
    ...
    local guid = Util.GetReadableGUID(self._owner.states.unit)
    if guid and F.IsPlayer(guid) then
...
---@return number?
local function GetDisplayPowerPercent(unit)
    if not UnitPowerPercent then return end

    if CurveConstants and CurveConstants.ScaleTo100 then
        return UnitPowerPercent(unit, nil, true, CurveConstants.ScaleTo100)
    end
...
---@param self PowerTextWidget
---@param unit UnitToken
local function SetSecretPowerPercent(self, unit)
    local powerPercent = GetDisplayPowerPercent(unit)
    if powerPercent == nil then
        self:SetText("")
        return
    end

    self:SetFormattedText("%d%%", powerPercent)
end
...
local function SetPower_Percentage(self, current, max, unit)
    if Util.HasAnySecretValues(current, max) then
        SetSecretPowerPercent(self, unit)
        return
    end
...
local function SetPower_Number(self, current, max)
    if not Util.IsValueNonSecret(current) then
        self:SetFormattedText("%s", current)
        return
    end
...
local function SetPower_Number_Short(self, current, max)
    if not Util.IsValueNonSecret(current) then
        if AbbreviateNumbers then
            self:SetFormattedText("%s", AbbreviateNumbers(current))
        else
            self:SetFormattedText("%s", current)
        end
        return
...
function W:CreatePowerText(button)
    ...
    local powerText = W.CreateBaseTextWidget(button, const.WIDGET_KIND.POWER_TEXT)
    ...
    function powerText:UpdateValue()
        ...
        if self.hideIfEmptyOrFull and Util.IsValueNonSecret(power) and Util.IsValueNonSecret(powerMax)
            and (power == 0 or power == powerMax) then
            self:Hide()
            return
        end

        if (not Util.IsValueNonSecret(powerMax)) or powerMax > 0 then
            button.widgets.powerText:SetValue(power, powerMax, unit)
