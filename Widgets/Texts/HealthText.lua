...
local function GetHealthInfo(unit, absorbs, healAbsorbs)
    ...
    if totalAbsorbs == nil then totalAbsorbs = 0 end
    if healAborbs == nil then healAborbs = 0 end
...
---@param self HealthTextWidget
---@param unit UnitToken
local function SetSecretHealthPercent(self, unit)
    local healthPercent = GetDisplayHealthPercent(unit)
    if healthPercent == nil then
        self:SetText("")
        return
    end

    self:SetFormattedText("%d%%", healthPercent)
end

---@param self HealthTextWidget
---@param value number
local function SetSecretNumber(self, value)
    self:SetFormattedText("%s", value)
end

---@param self HealthTextWidget
---@param value number
local function SetSecretShortNumber(self, value)
    if AbbreviateNumbers then
        self:SetFormattedText("%s", AbbreviateNumbers(value))
    else
        self:SetFormattedText("%s", value)
    end
end
...
local function SetHealth_Percentage(self, current, max, totalAbsorbs, healAbsorbs, unit)
    if Util.HasAnySecretValues(current, max) then
        SetSecretHealthPercent(self, unit)
        return
    end
...
---@param max number
---@param totalAbsorbs number
local function SetHealth_Percentage_Absorbs(self, current, max, totalAbsorbs, healAbsorbs, unit)
    if Util.HasAnySecretValues(current, max, totalAbsorbs) then
        SetSecretHealthPercent(self, unit)
        return
    end
...
---@param max number
---@param totalAbsorbs number
local function SetHealth_Percentage_Absorbs_Merged(self, current, max, totalAbsorbs, healAbsorbs, unit)
    if Util.HasAnySecretValues(current, max, totalAbsorbs) then
        SetSecretHealthPercent(self, unit)
        return
    end
...
---@param max number
---@param totalAbsorbs number
local function SetHealth_Percentage_Deficit(self, current, max, totalAbsorbs, healAbsorbs, unit)
    if Util.HasAnySecretValues(current, max) then
        self:SetText("")
        return
    end
...
local function SetHealth_Number(self, current, max, totalAbsorbs)
    if not Util.IsValueNonSecret(current) then
        SetSecretNumber(self, current)
        return
    end
...
local function SetHealth_Number_Short(self, current, max, totalAbsorbs)
    if not Util.IsValueNonSecret(current) then
        SetSecretShortNumber(self, current)
        return
    end
...
local function SetHealth_Number_Absorbs_Short(self, current, max, totalAbsorbs)
    if Util.HasAnySecretValues(current, totalAbsorbs) then
        if Util.IsValueNonSecret(totalAbsorbs) and totalAbsorbs == 0 then
            SetSecretShortNumber(self, current)
        else
            local currentText = AbbreviateNumbers and AbbreviateNumbers(current) or current
            local absorbText = AbbreviateNumbers and AbbreviateNumbers(totalAbsorbs) or totalAbsorbs
            self:SetFormattedText("%s+%s", currentText, absorbText)
        end
        return
    end
...
---@param max number
---@param totalAbsorbs number
local function SetHealth_Current_Short_Percentage(self, current, max, totalAbsorbs, healAbsorbs, unit)
    if Util.HasAnySecretValues(current, max) then
        SetSecretShortNumber(self, current)
        return
    end
...
local function SetHealth_Absorbs_Only(self, current, max, totalAbsorbs)
    if not Util.IsValueNonSecret(totalAbsorbs) then
        SetSecretNumber(self, totalAbsorbs)
        return
    end
...
local function SetHealth_Absorbs_Only_Short(self, current, max, totalAbsorbs)
    if not Util.IsValueNonSecret(totalAbsorbs) then
        SetSecretShortNumber(self, totalAbsorbs)
        return
    end
...
local function SetHealth_Absorbs_Only_Percentage(self, current, max, totalAbsorbs)
    if Util.HasAnySecretValues(totalAbsorbs, max) then
        self:SetText("")
        return
    end
...
function W:CreateHealthText(button, custom)
    ...
    function healthText:UpdateValue()
        ...
        if Util.IsValueNonSecret(healthMax) and healthMax == 0 then return end

        local isDeadOrGhost = UnitIsDeadOrGhost(unit)

        if self.hideIfFull and Util.IsValueNonSecret(health) and Util.IsValueNonSecret(healthMax) and health == healthMax then
            self:Hide()
            return
        end

        if isDeadOrGhost or (Util.IsValueNonSecret(health) and health == 0) then
            if self.hideIfEmpty then
                self:Hide()
                return
            end

            if self.showDeadStatus and isDeadOrGhost then
                self:SetText(L["Dead"])
                self:Show()
                return
            end
        ...
        self:SetValue(health, healthMax, totalAbsorbs, healAbsorbs, unit)
        self:Show()
    end
