...
local function UpdatePower(button, event, unit, powerType)
    ...
    local secretPower = not Util.IsValueNonSecret(power) or not Util.IsValueNonSecret(altPowerBar.maxPower)

    if not secretPower and altPowerBar.hideIfEmpty and power == 0 then
        altPowerBar:Hide()
    elseif not secretPower and altPowerBar.hideIfFull and power == altPowerBar.maxPower then
        altPowerBar:Hide()
    else
        altPowerBar:Show()
