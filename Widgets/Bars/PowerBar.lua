...
local function UpdatePower(button, event, unit, powerType)
    ...
    local secretPower = not Util.IsValueNonSecret(power) or not Util.IsValueNonSecret(powerBar.max)

    if not secretPower and powerBar.hideIfEmpty and power == 0 then
        powerBar:Hide()
    elseif not secretPower and powerBar.hideIfFull and power == powerBar.max then
        powerBar:Hide()
...
local function UpdatePowerMax(button, event, unit, powerType)
    ...
    if Util.IsValueNonSecret(powerMax) and powerMax < 0 then powerMax = 0 end
