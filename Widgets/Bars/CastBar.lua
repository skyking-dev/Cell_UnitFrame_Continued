...
local STATUSBAR_TIMER_DIRECTION_REMAINING = Enum and Enum.StatusBarTimerDirection and Enum.StatusBarTimerDirection.RemainingTime
...
---@param incoming any
---@return boolean
local function IsDifferentValue(current, incoming)
    if incoming == nil then
        return false
    end

    if not Util.IsValueNonSecret(current) or not Util.IsValueNonSecret(incoming) then
        return true
    end
...
---@param incoming any
---@return boolean
local function IsMismatchedValue(current, incoming)
...
---@return string
local function GetCastLabel(displayName, spellName)
    if displayName ~= nil and Util.IsValueNonSecret(displayName) and displayName ~= "" then
        return displayName
    end
...
---@param castBar CastBarWidget
---@param unit UnitToken
---@return boolean
local function ApplyDurationObject(castBar, unit)
    if not (Cell.isMidnight and castBar.statusBar and castBar.statusBar.SetTimerDuration) then
        return false
    end

    local durationObject
    if castBar.empowering and UnitEmpoweredChannelDuration then
        durationObject = UnitEmpoweredChannelDuration(unit)
    elseif castBar.channeling and UnitChannelDuration then
        durationObject = UnitChannelDuration(unit)
    elseif castBar.casting and UnitCastingDuration then
    ...
    castBar._durationObject = durationObject
    castBar.startTime = nil
    castBar.max = 1
    castBar.duration = castBar.channeling and 1 or 0
    castBar:SetMinMaxValues(0, 1)
    castBar.statusBar:SetTimerDuration(durationObject, STATUSBAR_INTERPOLATION_NONE,
        castBar.channeling and STATUSBAR_TIMER_DIRECTION_REMAINING or STATUSBAR_TIMER_DIRECTION_ELAPSED)
    if castBar.statusBar.SetToTargetValue then
        castBar.statusBar:SetToTargetValue()
    end
    if castBar.timerText then
        castBar.timerText:SetText("")
    end
...
local function UpdateElements(self)
    ...
    if self.empowering and self.showEmpowerSpellName then
        name = GetCastLabel(self.displayName, self.spellName)
    elseif self.spellText.enabled then
        name = GetCastLabel(self.displayName, self.spellName)
    end

    if self.spellText.showTarget and self.targetName ~= nil then
...
function CastStart(button, event, unit, castGUID)
    ...
    if name == nil then
        ...
        local isEmpowerCast = numStages ~= nil and Util.IsValueNonSecret(numStages) and numStages > 0
        event = isEmpowerCast and "UNIT_SPELLCAST_EMPOWER_START" or "UNIT_SPELLCAST_CHANNEL_START"
    end

    if name == nil or (castBar.onlyShowInterrupt and Util.IsValueNonSecret(notInterruptible) and notInterruptible) then
        castBar:HideCastBar()
    ...
    if IsDifferentValue(castBar.castGUID, castGUID) then
    ...
    elseif not ApplyDurationObject(castBar, unit) then
        castBar:HideCastBar()
        return
    end
    ...
    if castBar.empowering and numStages ~= nil and Util.IsValueNonSecret(numStages) then
...
function CastUpdate(button, event, unit, castID, spellID)
    ...
    if not castBar:IsShown()
        or IsMismatchedValue(castBar.castID, castID)
        or IsMismatchedValue(castBar.spellID, spellID) then
        return
    end
    ...
    if castBar.onlyShowInterrupt and Util.IsValueNonSecret(notInterruptible) and notInterruptible then
        castBar:HideCastBar()
    ...
    if castBar.timeToHold > 0 then
        ...
        castBar.SetSpellWidth(castBar.spellText,
            (castBar.interruptedLabel:gsub("%%t", type):gsub("%%s",
                GetCastLabel(castBar.displayName, castBar.spellName))),
...
local function SetCastBarColor(self)
    ...
    elseif Util.IsValueNonSecret(self.notInterruptible) and self.notInterruptible then
