...
function U:UnitFrame_UpdateHealthColor(button, fullUpdate)
    ...
    local healthPct = 0
    if not Util.HasAnySecretValues(cur, max) and max > 0 then
        healthPct = cur / max
    elseif type(button.states.healthPercent) == "number" then
        healthPct = button.states.healthPercent
    end
    ...
    if button:HasWidget(const.WIDGET_KIND.HEAL_PREDICTION) then
        button.widgets.healPrediction:UpdateStyle()
    end
...
local function UpdateUnitHealthState(button)
    ...
    if button.states.totalAbsorbs == nil then
        button.states.totalAbsorbs = 0
    end

    if not Util.HasAnySecretValues(health, healthMax) then
        health = math.min(health, healthMax) --! diff
        button.states.health = health
        button.states.healthPercent = healthMax > 0 and (health / healthMax) or 0
    else
        button.states.healthPercent = 0
    end

    button.states.wasDead = button.states.isDead
    if Util.IsValueNonSecret(health) then
        button.states.isDead = health == 0
    else
        button.states.isDead = UnitIsDeadOrGhost(unit) or false
    end
...
local function UpdateHealth(button)
    ...
    if not Cell.isMidnight
        and CellDB["appearance"]["barAnimation"] == "Smooth"
...
local function UpdateHealthMax(button)
    ...
    local healthMax = button.states.healthMax
    if Util.IsValueNonSecret(healthMax) and healthMax <= 0 then
        healthMax = 1
        button.states.healthMax = healthMax
    end
